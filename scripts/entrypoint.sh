...
HYTALE_BACKUP_ARCHIVE_MAX_COUNT="${HYTALE_BACKUP_ARCHIVE_MAX_COUNT:-}"
...
HYTALE_CLIENT_PID="${HYTALE_CLIENT_PID:-}"
...
HYTALE_SKIP_MOD_VALIDATION="${HYTALE_SKIP_MOD_VALIDATION:-false}"
...
/usr/local/bin/hytale-config-patch
...
if [ -n "${HYTALE_BACKUP_ARCHIVE_MAX_COUNT:-}" ]; then
  set -- "$@" --backup-archive-max-count "${HYTALE_BACKUP_ARCHIVE_MAX_COUNT}"
fi
...
if [ -n "${HYTALE_CLIENT_PID:-}" ]; then
  set -- "$@" --client-pid "${HYTALE_CLIENT_PID}"
fi
...
if [ -n "${HYTALE_MIGRATE_WORLDS:-}" ]; then
  set -- "$@" --migrate-worlds "${HYTALE_MIGRATE_WORLDS}"
fi

if [ -n "${HYTALE_MIGRATIONS:-}" ]; then
  set -- "$@" --migrations "${HYTALE_MIGRATIONS}"
fi
...
if is_true "${HYTALE_SKIP_MOD_VALIDATION}"; then
  set -- "$@" --skip-mod-validation
fi
